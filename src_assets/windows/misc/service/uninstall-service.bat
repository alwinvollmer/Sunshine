...
sc qc SunshineExtendedService >nul 2>&1
if %ERRORLEVEL%==0 (
    if not exist "%SERVICE_CONFIG_DIR%\" mkdir "%SERVICE_CONFIG_DIR%\"

    rem Get the start type
    for /f "tokens=3" %%i in ('sc qc SunshineExtendedService ^| findstr /C:"START_TYPE"') do (
        set "CURRENT_START_TYPE=%%i"
    )

    rem Set the content to write
    if "!CURRENT_START_TYPE!"=="2" (
        sc qc SunshineExtendedService | findstr /C:"(DELAYED)" >nul
        if !ERRORLEVEL!==0 (
...
rem Stop and delete the new SunshineExtendedService service
net stop SunshineExtendedService
sc delete SunshineExtendedService
